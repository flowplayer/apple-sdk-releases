...
To run the example apps included in this repository, you need an access token from Wowza Flowplayer or Wowza Video. This token allows the player to display content in your iOS or tvOS application. You can update your target's **Info.plist** file  with this token, or you can accomplish the same goal by updating the `ACCESS_TOKEN` in the `AppDelegate` method included in the example app configurations.
